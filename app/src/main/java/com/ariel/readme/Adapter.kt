...
class Adapter(private val theList:  List<Chat>) :
    ...
    override fun onBindViewHolder(holder: ChatHolder, position: Int) {
        ...
        holder.TheUserName.text=theList[position].toString()
    }
       ...
       //Remains the part of clicking chat
        init {
            ...
            ButDeleteChat.setOnClickListener {
                ...
                val text : String = theList[position].toString()
                //if the user click in the garb ask them if he is sure
                builder.setTitle("Are you sure!")
                builder.setMessage("are you sure you want to remove the $text chat?")
                builder.setPositiveButton("Yes") { _: DialogInterface, _: Int ->
                    ChatRepository().removeChat(text, AuthService.getCurrentFirebaseUser()!!.uid)
                }
